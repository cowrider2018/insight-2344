...
- **聚焦池（~50 檔半導體同儕，`--tdcc-pool` / `--tdcc`）— 訊號明顯更強且加入真大戶因子有效**：同質同業的橫斷面排序更乾淨。
  ...
  - → **大戶週變化貢獻真實邊際**（IC 0.030→0.036、毛 +10pp、近期季 IC 0.028→0.036），證實「在同業橫斷面內，籌碼+大戶結構是可survive成本的 alpha」。
  - 保留：42 檔（~8 檔資料不足剔除）、每分位僅 ~8 檔→集中度高；228 日單一 regime；未含真實成本/借券/衝擊；屬「強而有望、待更長區間與純做多/實際成本複核」。
