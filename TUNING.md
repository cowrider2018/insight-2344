...
## 0. 一句話總結
十面（技術 / 籌碼 / 消息 / 基本 / 美光 / 費半 / **日內 1 分 K** / **主力分點** / **TDCC 千張大戶** / **外資台指期未平倉**）各自評分 → 加權合成方向 → 與當日實際漲跌（中性帶 ±1%）比對 →
...
| 檔案 | 角色 |
|---|---|
| `src/timeline_db.py` | SQLite 時間軸 DB `data/market.db`：`news/chips/revenue/candles/us_market/candles_1min/broker_branches/branch_wf/tdcc_holders` 表 + 查詢（`*_asof` 皆 `< D`，無 look-ahead；TDCC 以**公布日 avail_date** 比較） |
| `src/fetch_tdcc.py` | **第九面（TDCC 千張大戶）**：OpenData CSV（當週全市場）＋ smart.tdcc 查詢（歷史逐週，synchronizer-token 流程）→ 大戶/中實/散戶占比 |
| `src/fetch_taifex.py` | **第十面（外資台指期未平倉）**：TAIFEX 三大法人 CSV 下載（一次取整段、big5）→ 外資 TXF 多空未平倉口數淨額 |
...
- **無 look-ahead**：技術只用 D-1 收盤、籌碼 `data_date < D`、消息 `published_at < D 09:00`、美股 `date < D`、日內 1 分 K `date < D`（取 D-1 整日盤中）、TDCC 大戶 **公布日 `avail_date < D`**（資料日為週結算、公布有 lag，故以 avail_date=data_date+lag 比較）、外資台指期未平倉 `date < D`（盤後公布取 D-1）；`backtest.extract_features` 內以 `assert` 強制，**不可拿掉**。
