...
## 0. 一句話總結
九面（技術 / 籌碼 / 消息 / 基本 / 美光 / 費半 / **日內 1 分 K** / **主力分點** / **TDCC 千張大戶**）各自評分 → 加權合成方向 → 與當日實際漲跌（中性帶 ±1%）比對 →
...
- **無 look-ahead**：技術只用 D-1 收盤、籌碼 `data_date < D`、消息 `published_at < D 09:00`、美股 `date < D`、日內 1 分 K `date < D`（取 D-1 整日盤中）、TDCC 大戶 **公布日 `avail_date < D`**（資料日為週結算、公布有 lag，故以 avail_date=data_date+lag 比較）；`backtest.extract_features` 內以 `assert` 強制，**不可拿掉**。
